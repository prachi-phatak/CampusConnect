...
serve(async (req) => {
  ...
  try {
    ...
    const { data: members, error: membersError } = await supabase.rpc("get_event_member_emails", {
      p_event_id: event_id,
    });
    ...
    if (!resendApiKey) {
      ...
      return new Response(
        JSON.stringify({ message: "Mock emails sent successfully.", count: emailList.length }),
        {
          status: 200,
          headers: { ...corsHeaders, "Content-Type": "application/json" },
        },
